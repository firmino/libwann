  ...
  maxMemoryValue(maxMemoryValue)
{
    ...
    if(!hasRestMemory)
    {
        for(int i=0; i<numMemories; i++)
            memories.push_back(new Memory(numBits, maxMemoryValue, isCummulative, ignoreZeroAddr));
    }
    ...
    else
    {
        ...
        //the rest memory 
        memories.push_back(new Memory((retinaLength % numBits), maxMemoryValue, isCummulative, ignoreZeroAddr));
    }
}
...
void Discriminator::addOnlineTrainning(const vector<int> &retina)
{
    ...
    if(restOfPositions != 0)
    {
        ...
        lastMemoryPosition = memIndex + 1;
        memories[lastMemoryPosition]->decrease();
        memories[lastMemoryPosition]->addOnlineValue(addr, 1);
    }
}
